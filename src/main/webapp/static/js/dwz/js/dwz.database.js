...
(function($){
	...
	$.extend({
		bringBackSuggest: function(args){
			...
			//根据class选择器 对查询带回中  放款方式 下拉方法的触发。
			$box.find(".changeSelect").each(function(){
				var $select = $(this), selectName = $select.attr("name");
				for (var key in args) {
					var name = (_lookup.pk == key) ? _util.lookupPk(key) : _util.lookupField(key);
					if (key!=null&&key!=""&&selectName!=null&&selectName!=''&& name == "dwz."+selectName ) {
						 $select.val(args[key]);
						 $select.change();
						break;
					}
		...
		suggest: function(){
			...
			return this.each(function(){
				...
				function _show(event){
					...
					//global:false  阻止【正在加载中..】 显示。
					$.ajax({
						type:'POST', dataType:"json", url:url, cache: false,global:false,
		...
		itemDetail: function(){
			return this.each(function(){
				...
				if (addButTxt) {
					...
					var $rowNum = $('<input type="text" name="dwz_rowNum" class="textInput" style="margin:2px;" value="1" size="2"/>条数据').insertBefore($table);
				...
				$this.click(function(){
					...
					function _doPost(){
						$.ajax({
							...
							success: $this.attr("targetType") == "dialog" ? dialogAjaxDone : navTabAjaxDone,
							error: DWZ.ajaxError
						});
					}
