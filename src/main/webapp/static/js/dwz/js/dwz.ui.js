...
function initUI(_box){
	...
	// 这里放其他第三方jQuery插件...
	if ($.fn.dynamicDataGrid) $("table.dynamicDataGrid", $p).dynamicDataGrid();	//动态可编辑DataGrid
	
	if (window._initUICallbacks) {
		$.each(_initUICallbacks, function() {
			this.call($p);
		});
	}	
	// 这里放其他第三方jQuery插件...
	$(".control_box", $p).myButton();
}
