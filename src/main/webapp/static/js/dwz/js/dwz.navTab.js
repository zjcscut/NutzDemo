...
var navTab = {
    ...
    _init: function () {
        ...
        this._getTabs().each(function (iTabIndex) {
            ...
            $(this).find(navTab._op.close$).unbind("click").click(function () {
                ...
                var nowId = $this._getTabs().eq(iTabIndex).attr("tabid");
                var value = null;
                var bFind = false;

                if (iTabIndex == $this._currentIndex) {	//选中当前执行关闭

                    if ($this.isEmpty($this._jsonId)) {
                        $this._closeTab(iTabIndex);
                    } else {
                        for (var name in $this._jsonId) {
                            if (name == nowId) {
                                bFind = true;
                                value = $this._jsonId[name];
                            }
                        }

                        if (bFind) {
                            $this._closeTab(iTabIndex, value);
                        } else {
                            $this._closeTab(iTabIndex);
                        }
    ...
    //切换到哪一个（iTabIndex为切换的索引）
    _switchTab: function (iTabIndex) {

        //先将navTab上所有的标签的selected移除，然后给索引为iTabIndex加上selected
        var $tab = this._getTabs().removeClass("selected").eq(iTabIndex).addClass("selected");

        //navTab-panel下所有page unitBox（每个标签相对应的显示内容div）都隐藏，只显示索引为iTabIndex的
    ...
    closeCurrentTab: function (openTabid) {
        if (this._currentIndex > 0) {
            ...
            //关闭按钮返回
            var nowId = this._getTabs().eq(this._currentIndex).attr("tabid");
            var value = null;
            var bFind = false;
            ...
            if (this.isEmpty(this._jsonId)) {
                ...
                if (bFind) {
                    this._closeTab(this._currentIndex, value);
                } else {
                    this._closeTab(this._currentIndex);
                }
    ...
    reloadFlag: function (tabid) {
        ...
        for (var i = 0; i < ids.length; i++) {
            var $tab = this._getTab(ids[i]);
            if ($tab) {
                if (this._indexTabId(ids[i]) == this._currentIndex) this._reload($tab, true);
    ...
    openTab: function (tabid, url, options) { //if found tabid replace tab, else create a new tab.
        ...
        // 记录互相有联系的tabid
        var oldId = this._getTabs().eq(this._currentIndex).attr("tabid");

        var nowId = tabid;

        var oDiv = this._getPanels().eq(this._currentIndex).find("*");

        for (var i = 0; i < oDiv.length; i++) {
            if (oDiv.eq(i).attr("rel") == nowId) {
                this._jsonId[nowId] = oldId;
            }
        }
