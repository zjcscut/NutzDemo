...
//					$.pdialog.switchDialog(dialog);
		...
		_init:function(dialog, options) {
			...
			//modify by zhongmin 增加dialog兼容百分比设置
			op.height = this.convertPercent(op.height,$(window).height());
			op.width = this.convertPercent(op.width,$(window).width());
		...
		// modify by zhongmin 兼容百分比设置
		convertPercent:function(percent,windowPx){

			var rs=this.regx.test(percent);
			if(rs){
				return (parseFloat(percent)/100)*windowPx;
			}else{
				return percent;
			}
