 ...
 * @check luxh
 * @checkDate 2016-03-17
 * @checkDesc 将find 方法传入的Integer类型的参数转换为long类型
 */
@IocBean
public class BasicDao {
     ...
     * @return 查询到的对象
     */
    public <T> T find(Integer id, Class<T> c) {
        return dao.fetch(c, Long.valueOf(id));
    }
     ...
     * @return
     */
    public <T> List<T> search(String tableName, Condition condition) {
        return (List<T>) dao.query(tableName, condition, null);
    }
     ...
     * @return
     */
    public <Record> List<Record> searchBySql(String sqlstr) {
        Sql sql = Sqls.queryRecord(sqlstr);
        dao.execute(sql);
        List list = (List) sql.getResult();
        return list;
    }
     ...
     * @return List
     */
    public <T> List<T> searchByPage(Class<T> c, int currentPage, int pageSize, String orderby, String orderType) {
        if (currentPage == 0) currentPage = 1;
        if (pageSize == 0) pageSize = SystemConst.PAGE_SIZE;
        Pager pager = dao.createPager(currentPage, pageSize);
        if (orderType != null && orderType.equals("asc"))
            return dao.query(c, Cnd.orderBy().asc(orderby), pager);
        else
     ...
     * @return true 修改成功,false 修改失败
     */
    public <T> boolean updateNotIgnoreNull(T t) {
    ...
    public <T> void deleteByIds(Class<T> c, String ids) {
        ...
        if (null != entity.getNameField()) {
            id = entity.getNameField().getColumnName();
        } else {
            id = entity.getIdField().getColumnName();
        }
     ...
     *
     * @param <T>
     * @param fieldName 匹配名称
     * @param value     匹配值
     * @return T
     */
    public <T> T findByCondition(Class<T> c, String fieldName, String value) {
        return dao.fetch(c, Cnd.where(fieldName, "=", value));
    }
